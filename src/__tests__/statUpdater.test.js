import {enterStat, statUpdater} from '../reducers/statUpdater.js'
describe('testing stats-start end, rest',()=>{
    ...
    test('start',()=>{
        let data = {timeStart:0,timeEnd:0,keyCounter:0,correctKey:0}
        let action = enterStat('start','1')
    ...
    test('reset',()=>{
        let data = {timeStart:234,timeEnd:123,keyCounter:2,correctKey:3}
        let result = {timeStart:0,timeEnd:0,keyCounter:0,correctKey:0}
...
describe('testing stats-key',()=>{
    test('enter key - correct',()=>{
        let data = {timeStart:234,timeEnd:123,keyCounter:2,correctKey:3}
        let result = {timeStart:234,timeEnd:123,keyCounter:3,correctKey:4}
        let action = enterStat('key',1)
        expect(statUpdater(data,action)).toEqual(result)
    })
    test('enter key - incorrect',()=>{
        let data = {timeStart:234,timeEnd:123,keyCounter:2,correctKey:3}
        let result = {timeStart:234,timeEnd:123,keyCounter:3,correctKey:3}
...
describe('testing stats-backspace',()=>{
    test('backspace last correct -non space',()=>{
        let data = {timeStart:234,timeEnd:123,keyCounter:2,correctKey:3}
        let result = {timeStart:234,timeEnd:123,keyCounter:3,correctKey:2}
        let action = enterStat('backtrackstat',[1,'','a'])
        expect(statUpdater(data,action)).toEqual(result)
    })
    test('backspace last correct - space',()=>{
        let data = {timeStart:234,timeEnd:123,keyCounter:2,correctKey:2}
        let result = {timeStart:234,timeEnd:123,keyCounter:3,correctKey:1}
    ...
    test('default',()=>{
      const action = enterStat("asf","")
        const data = {error:'afasdf', index: 80, arr: [], last_char: true}
        expect(statUpdater(data,action))
        .toEqual(data)
    })
})